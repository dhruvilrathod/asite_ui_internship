...
let approvedBlogsArr: any;
if (approvedBlogs) {
  approvedBlogsArr = JSON.parse(approvedBlogs);
}

const deleteBlog = (blogIndex: any) => {
  if (window.confirm("Are you sure want to Delete this Blog ?")) {
    const popedBlog = approvedBlogsArr.splice(blogIndex, 1);
...
if (approvedBlogsArr.length > 0) {
  for (let i = 0; i < approvedBlogsArr.length; i++) {
    ...
    singleBlog.innerHTML = `<img src="${i + 1}.png" alt="Nature">
      <div class="blogDetails">
        <div class="user">
          ...
          <div class="details">
            <p>
              ${approvedBlogsArr[i].fname} ${approvedBlogsArr[i].lname}
            </p>
            <p>
              ${approvedBlogsArr[i].country}
            </p>
          </div>
        </div>
  
        <h3>${approvedBlogsArr[i].title}</h3>
        <h5>Blog description</h5>
        <p class="desc">
          ${approvedBlogsArr[i].description}
        </p>
        <div class="btnBar">
          <button onclick="deleteBlog(${i})" class="reject">
            Delete
          </button>
        </div>
      </div>`
